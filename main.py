...
# 检测版本是否为最新版
if tfcheckupdate == 'True':
    vs = requests.get("http://update.mcbeserver.cn:333/MCQQRobot/version.html")
    if vs.text != appversion:
        tkinter.Tk().withdraw()
        tkinter.messagebox.showinfo(title='有新的版本可以更新！', message='有新版本：' + vs.text + '\n请前往QQ群159323818下载')
...
if qs == '2':
    ...
    qqowner = input('请输入机器人的主人(只有此人可以使用开关服，发送指令到服务器功能，仅支持一人)')
    ...
    if fh2["msg"] == "success":
        ...
        print('绑定过程中出现错误，请检查配置是否正常，有疑问请加入QQ群:159323818')
    ...
    if vs.text != appversion:
        ...
        tkinter.Tk().withdraw()
        tkinter.messagebox.showinfo(title='当前软件版本为最新', message='当前软件版本为最新')


elif qs == '1':
    if BlackBEenable == 'true':
        print('[MCQQRobot]云黑功能已启用')
    else:
        print('[MCQQRobot]云黑功能已关闭')
    if ETWenable == "true":
        print("[MCQQRobot]邮件功能已启用")
    else:
        print("[MCQQRobot]邮件功能已关闭")
    url = 'http://' + sh + ':' + sp + "/auth"
    data1 = {"authKey": sa}
    r = requests.session()
    s = r.post(url, json=data1)
    ...
    fh2 = json.loads(s.text)
    if fh2["msg"] == "success":
        print('[MCQQRobot]机器人绑定成功')
        url = 'ws://' + sh + ':' + sp + "/message?sessionKey=" + sk
        print("[MCQQRobot]机器人对接成功，已启用！")
    else:
        print('[MCQQRobot]绑定过程中出现错误，请检查配置是否正常，有疑问请加入QQ群:159323818')
    ...
    async def startup(uri):
        async with AioWebSocket(uri) as aws:
            ...
            while True:
                ...
                mes = json.loads(mes)
                print('\n' + '[Mirai]' + '{time}-接收到: {mes}'
                      .format(time=datetime.now().strftime('%Y-%m-%d %H:%M:%S'), mes=mes))
                mst = mes['type']
                if mst == 'GroupMessage':
                    si = mes['sender']
                    si2 = si["group"]
                    si3 = si2["id"]
                    if re.search(str(si3), sg) and BlackBEenable == 'true':
                        i1 = mes['messageChain']
                        sendq = mes['sender']
                        sendqq = sendq['id']
    ...
    if __name__ == '__main__':
        remote = 'ws://' + sh + ':' + sp + "/all?sessionKey=" + sk
        try:
